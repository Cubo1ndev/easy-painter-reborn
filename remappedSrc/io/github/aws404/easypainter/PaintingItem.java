...
public class PaintingItem extends DecorationItem {
    ...
    public Text getName(ItemStack stack) {
        ...
        if (stack.getTag() != null && stack.getTag().contains("EntityTag")) {
            ...
            text.append(new TranslatableTextContent("item.easy_painter.painting.set", EasyPainter.getPaintingDisplayName(current).formatted(Formatting.ITALIC)));
        }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (user.isSneaking()) {
            ...
            NbtCompound entityTag = stack.getOrCreateSubTag("EntityTag");
            entityTag.putString("Motive", Registry.PAINTING_MOTIVE.getId(PaintingVariant.ALBAN).toString());
        }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (playerEntity != null && !this.canPlaceOn(playerEntity, direction, itemStack, blockPos2)) {
            ...
            if (paintingEntity.canStayAttached()) {
                ...
                playerEntity.sendMessage(new TranslatableTextContent("message.easy_painter.painting_cant_fit").formatted(Formatting.RED), true);
                return ActionResult.FAIL;
            }
        }
    }
