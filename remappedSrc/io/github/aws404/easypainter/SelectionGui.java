...
public class SelectionGui extends SimpleGui {
    ...
    private SelectionGui(PaintingEntity entity, List<PaintingVariant> motives, ServerPlayerEntity player) {
        ...
        this.entity = entity;
        this.setTitle(new TranslatableTextContent("screen.easy_painter.title"));
        ...
        for (PaintingVariant possibility : motives) {
            GuiElementBuilder builder = new GuiElementBuilder(Items.PAINTING)
                    ...
                    .addLoreLine(new LiteralText("")
                            .append(new TranslatableTextContent("screen.easy_painter.bullet").formatted(Formatting.GOLD))
                            .append(new TranslatableTextContent("screen.easy_painter.size",
                                    new TranslatableTextContent("screen.easy_painter.size.data", possibility.getHeight() / 16, possibility.getWidth() / 16).formatted(Formatting.WHITE)
                            ).formatted(Formatting.YELLOW))
                    )
            ...
            if (possibility == entity.motive) {
                ...
                builder.addLoreLine(new TranslatableTextContent("screen.easy_painter.currently_selected").formatted(Formatting.GRAY));
                builder.glow();
            }
    ...
    public static SelectionGui createGui(PaintingEntity entity, ServerPlayerEntity player) {
        List<PaintingVariant> motives = Registry.PAINTING_MOTIVE.stream().filter(motive -> EasyPainter.canPaintingAttach(entity, motive)).sorted(Comparator.comparingInt(o -> o.getHeight() * o.getWidth())).collect(Collectors.toList());
